...
from .datasplit import getdataloader
...
def get_data(data_name):
    ...
    datalist = {'cifar10': 'cifar10', 'cifar100': 'cifar100',
                'officehome': 'img_union', 'pacs': 'img_union', 'vlcs': 'img_union', 'medmnist': 'medmnist',
...
class CIFAR10Dataset(Dataset):
    def __init__(self, filename='', transform=None):
        self.transform = transform
        train = True
        cifar10 = CIFAR10(filename, train=train, transform=ToTensor(), download=True)

        self.data = torch.tensor(cifar10.data).permute(0, 3, 1, 2)  # Change shape to (N, C, H, W)
...
def getfeadataloader(args):
    ...
    for item in args.domains:
        ...
        l1, l2, l3 = int(l * args.datapercent), int(l * 0.2), int(l*0.1)
        ...
        trd.append(torch.utils.data.DataLoader(
            trl[-1], batch_size=args.batch_size, shuffle=True))
        vad.append(torch.utils.data.DataLoader(
            val[-1], batch_size=args.batch_size, shuffle=False))
        ted.append(torch.utils.data.DataLoader(
            tel[-1], batch_size=args.batch_size, shuffle=False))
    args.n_samples = np.sum([trd[i].dataset.indices.size for i in range(args.n_train_clients)])
    return trd, vad, ted
...
def getlabeldataloader(args, data):
    ...
    for i in range(len(trl)):
        trd.append(torch.utils.data.DataLoader(
            trl[i], batch_size=args.batch_size, shuffle=True))
        vad.append(torch.utils.data.DataLoader(
            val[i], batch_size=args.batch_size, shuffle=False))
        ted.append(torch.utils.data.DataLoader(
            tel[i], batch_size=args.batch_size, shuffle=False))
    return trd, vad, ted


def medmnist(args):
    print(args.root_dir+args.dataset+'/')
...
def cifar10(args):
    print(args.root_dir+args.dataset+'/')
    data = CIFAR10Dataset(args.root_dir+args.dataset+'/')
    trd, vad, ted = getlabeldataloader(args, data)
    args.num_classes = 11
    return trd, vad, ted
