...
def get_model(prm, model_type='Stochastic'):
    ...
    if model_name == 'FcNet3':
        ...
        from Models import get_bayes_densenet_model_class
        densenet_model = get_bayes_densenet_model_class(prm, task_info)
        model = densenet_model(depth=20)

    elif model_name == 'ConvNet4':
        model = ConvNet4(model_type, model_name, linear_layer, conv2d_layer, task_info)
...
def get_info(prm):
    if prm.dataset == 'MNIST':
        info = {'input_shape': (1, 28, 28),  'n_classes': 10}

    elif prm.dataset == 'medmnist':
        info = {'input_shape': (1, 28, 28),  'n_classes': 11}

    elif prm.dataset == 'pacs':
        info = {'input_shape': (3, 224, 224), 'n_classes': 7}

    elif prm.dataset == 'cifar10':
        info = {'input_shape': (3, 32, 32), 'n_classes': 10}

    elif prm.dataset == 'Omniglot':
        info = {'input_shape': (1, 28, 28), 'n_classes': prm.N_Way}

    elif prm.dataset == 'SmallImageNet':
